...
Finally, the detailed study of a specific observation can be extrapolated to the rest of the test set observations. This is due to the similarity in the shape of the distributions of the different observations and the appearance of heavy tails in each of them (Figures 10 and 11). Therefore, it is reasonable to think that the distribution of Dst conditioned on each of the features can be modeled by the same family of distributions. Given the resemblance of the shape of each of the approximate distribution functions to the sigmoid function, it is reasonable to think that the general law of $Y|X$ can be modeled by:
